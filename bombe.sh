...
#http://chart.finance.yahoo.com/table.csv?s=OIL&a=11&b=17&c=2016&d=0&e=17&f=2017&g=d&ignore=.csv
mv "table.csv?s=${codice_azione}&a=${mese_inizio}&b=${giorno_inizio}&c=${anno_inizio}&d=${mese_fine}&e=${giorno_fine}&f=${anno_fine}&g=${tipo}&ignore=.csv" tabella.csv
...
awk -F "\"*,\"*" '{print $7}' tabella.csv | sed '1d' | sed 's/^/,/g' | sed "1s/^,/[/" | sed '$s/$/]/' | sed 's/^/        /g' > ${codice_azione}.txt

# Cancello il csv perché è inutile
rm tabella.csv

cat bomba.hs > tmp.hs
cat ${codice_azione}.txt >> tmp.hs

ghc tmp.hs
./tmp > res_${codice_azione}.txt
rm tmp*
