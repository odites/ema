...
maxi xs = F.maximumBy (comparing fst) (S.zip xs sequenza_standard)

media xs = sum xs / (fromIntegral $ S.length xs)

corpo (viewl -> EmptyL) _ _ = empty
corpo (viewl -> x :< xs) y costante = valore <| corpo xs valore costante
    where valore = x * costante + y * (1 - costante)    

ema ampiezza lista = testa <| corpo sottostante testa costante
    where testa = media $ S.take ampiezza lista
          sottostante = S.drop ampiezza lista
...
incrocia (viewl -> x :< empty) _ = 1
incrocia _ (viewl -> y :< empty) = 1
...
tupla_massimale volume lista = maxi $ fmap (netto' volume lista) sequenza_standard
...
sequenza_standard = fromList [1..numero_dati]
dati = S.fromList
        [0.4141
        ,0.4168
        ,0.3934
